...
(function() {
  ...
  angular.module("FTravel.user").factory("UserFtr", userFtr);
  ...
  function userFtr(
    ...
    self.openSignin = function(sco) {
      ...
      if (!self.signinModal) {
        $ionicModal
          .fromTemplateUrl("app/user/sign-in.modal.html", {
            scope: sco
          })
          .then(function(modal) {
      ...
      sco.workPlaceTapped = function() {
        ...
        if (!sco.workPlaceModal) {
          $ionicModal
            .fromTemplateUrl("app/user/work-place-choose.modal.html", {
              scope: sco
            })
            .then(function(modal) {
    ...
    self.openActiveUser = function(sco) {
      ...
      if (!sco.activeUserModal) {
        $ionicModal
          .fromTemplateUrl("app/user/kich-hoat-tai-khoan.modal.html", {
            scope: sco,
            animation: "slide-in-up"
          })
          .then(function(modal) {
      ...
      sco.submitActiveUser = function() {
        self.active(sco.form_active_user).then(
          function(rep) {
    ...
    self.openForgetPass = function(sco) {
      ...
      sco.changeSendType = function(type) {
        sco.form_resetpass.type = type;
      };

      if (!sco.forgetPassModal) {
        $ionicModal
          .fromTemplateUrl("app/user/forget-password.modal.html", {
            scope: sco,
            animation: "slide-in-up"
          })
          .then(function(modal) {
      ...
      sco.checkCaptcha = function(straaa) {
        if (straaa == sco.iNumber) sco.isShow = true;
        else sco.isShow = false;
      };
      ...
      sco.submitResetpass = function(straaa) {
        ...
        self.resetPassword(sco.form_resetpass).then(
          function(rep) {
            // console.log(JSON.stringify(rep));
            sco.forgetPassModal.hide();
            Popup.s("Thông tin của bạn đã được gửi thành công, vui lòng thay đổi lại mật khẩu",function(ok) {
                if (ok) self.openChangeForgetPass(sco);
              });
          },
          function(err) {
            Popup.e(err);
    ...
    self.openChangeForgetPass = function(sco) {
          sco.form_updatepass = {
          screenname: "",
          tempcode: ""
      };
      if (!sco.modal) {
        $ionicModal.fromTemplateUrl("app/user/forget-password-update.view.html",{
              scope: sco,
              animation: "slide-in-up"
            }).then(function(modal) {
              sco.modal = modal;
      ...
      sco.updateForgetPassword = function(straaa) {
        // validation
        var _msg_err = "";
        if (sco.form_updatepass.password.length <= 0) _msg_err += "Chưa nhập mật khẩu mới</br>";
        if (sco.form_updatepass.tempcode.length <= 0)_msg_err += "Chưa nhập mã xác nhận</br>";
        if (_msg_err !== "") {
            Popup.e(_msg_err);
            return;
        }
        self.updateForgetPassword(sco.form_updatepass).then(
            function(rep) {
                // console.log(JSON.stringify(rep));
                Popup.a("Thay đổi mật khẩu thành công");
                sco.modal.hide();
                // $ionicHistory.goBack();
            },
            function(err) {
    ...
    self.setInfoFireBase = function(params){
      return API.call(API_CONST.user.registerFireBaseXLCV, params);
    }
    self.logIn = function(params) {
      return API.loginn(API_CONST.user.logIn, params);
    ...
    self.getInfos = function(params) {
      var deferred = $q.defer();
      API.getInfo(API_CONST.user.getInfo,params).then(
        function(rep) {
        deferred.resolve(rep);
         console.log(rep);
        },
    ...
    self.doiMatKhau = function(params) {
      return API.loginn(API_CONST.user.doiMatKhau, params);
    };
    // thay doi avatar
    self.thaydoiavatar = function (params){
      return API.call(API_CONST.user.saveavatar, params);
    };

    self.updateForgetPassword = function(params) {
      return API.call(API_CONST.user.updateForgetPassword, params);
    };

    self.updateInfo = function(params) {
      return API.call(API_CONST.user.updateInfo, params);
      
    }
    // Get Permission
    
    self.CheckPermissions = function(params) {    
      return API.loginn(API_CONST.user.getPermissions, params);
